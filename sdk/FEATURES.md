...
- [Pipeline features with custom Tekton implementation](#pipeline-features-with-custom-tekton-implementation)
  ...
  * [Features with different behaviors than Argo](#features-with-different-behaviors-than-argo)
...
these Kubernetes resources. The [resourceop_basic](/sdk/python/tests/compiler/testdata/resourceop_basic.py),
...
output to the `/tekton/results` directory. The [parallel_join](/sdk/python/tests/compiler/testdata/parallel_join.py) python test
is an example of how to use this feature.
...
as files before the main task is executed. The [input_artifact_raw_value](/sdk/python/tests/compiler/testdata/input_artifact_raw_value.py)
...
during execution. The [artifact_location](/sdk/python/tests/compiler/testdata/artifact_location.py) python test is an example of how to
use this feature.
...
Below are the features that has certain limitation in the current Tekton release.
...
## Features with different behaviors than Argo
Below are the KFP-Tekton features with different behaviors than Argo.
...
image field. Until this issue is resolved the best way to avoid it is to avoid
...
be quite confusing. However, it has no function impacts. 
...
one podTemplate allowed in each pipeline. However, in Argo these features are apply to one particular task. 

There is a pull request in Tekton to define podTemplate for individule task. Once that feature is available, then we need to reimplement this feature
to replicate the same Argo behaviors.
