...
def parse_arguments():
  ...
  parser.add_argument('--disable-cache',
                      action='store_true',
                      help='disable caching, default is enabled.')

  args = parser.parse_args()
  return args


def _compile_pipeline_function(pipeline_funcs, function_name, output_path, type_check, enable_cache):
  ...
  TektonCompiler().compile(pipeline_func, output_path, type_check, enable_cache)


def compile_pyfile(pyfile, function_name, output_path, type_check, enable_cache):
  sys.path.insert(0, os.path.dirname(pyfile))
  try:
    filename = os.path.basename(pyfile)
    with kfp_compiler_main.PipelineCollectorContext() as pipeline_funcs:
      __import__(os.path.splitext(filename)[0])
    _compile_pipeline_function(pipeline_funcs, function_name, output_path, type_check, enable_cache)
  finally:
    del sys.path[0]
...
def main():
    ...
    if args.py:
        compile_pyfile(args.py, args.function, args.output, not args.disable_type_check, not args.disable_cache)
    else:
