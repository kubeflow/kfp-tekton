...
def compile_pyfile(pyfile, function_name, output_path, type_check, enable_artifacts=False):
  sys.path.insert(0, os.path.dirname(pyfile))
  try:
    filename = os.path.basename(pyfile)
    with kfp_compiler_main.PipelineCollectorContext() as pipeline_funcs:
      __import__(os.path.splitext(filename)[0])
    _compile_pipeline_function(pipeline_funcs, function_name, output_path, type_check, enable_artifacts)
  finally:
    del sys.path[0]
...
def main():
    ...
    if args.py:
        compile_pyfile(args.py, args.function, args.output, not args.disable_type_check, args.enable_artifacts)
    else:
