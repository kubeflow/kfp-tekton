...
def fix_big_data_passing(
    ...
    7. Convert "_" to "-" in the params and artifacts.
    8. A PVC named with pipelinerun name will be created if bigger data passing, as workspaces need to use it.
    ...
    container_templates = [
        ...
        if template['kind'] == 'Task' and template.get('metadata', {}).get(
            'labels', {}).get('app') != 'resourceOp'
    ]
    pipeline_templates = [
        template for template in workflow if template['kind'] == 'Pipeline'
    ]
    resource_templates = [
        template for template in workflow if template.get('metadata', {}).get(
            'labels', {}).get('app') == 'resourceOp'
    ]
    ...
    # We replaced '_' to '-' for result and also other area since tekton don't support "_" for most areas.
    # So we need to make the format in tekton unified for params, artifacts, outputs.
    ...
    for pipeline in pipeline_templates:
        ...
        pipeline, pipeline_workspaces = bigger_data_passing_pipeline(
    ...
    if pipeline_workspaces:
        ...
        for pipelinerun in pipelinerun_templates:
            pipeline, pipelinerun_workspaces = bigger_data_passing_pipelinerun(
    ...
    for task_template in container_templates:
        task_template = bigger_data_passing_tasks(
...
def bigger_data_passing_pipeline(template: dict, inputs_tasks: set(),
                                 outputs_tasks: set):
    pipeline_workspaces = set()
...
def bigger_data_passing_pipelinerun(pr: dict, pw: set):
    prw = set()
...
def bigger_data_passing_tasks(task: dict, inputs_tasks: set,
    ...
    task['spec']['params'] = [
        ...
        if (task_name, task_parma.get('name')) not in inputs_tasks
    ]
