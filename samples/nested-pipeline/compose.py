...
class GetFrequentWordOp(dsl.ContainerOp):
  ...
  def __init__(self, name, message):
    ...
    super(GetFrequentWordOp, self).__init__(
        ...
        command=['bash', '-c'],
        arguments=['echo \'%s\' | tee /tmp/results.txt ; python -c "from collections import Counter; '
                  'foo=open(\'"/tmp/results.txt"\', \'r\').read();'
                  'words = Counter(foo.split()); print(max(words, key=words.get))" '
...
class SaveMessageOp(dsl.ContainerOp):
  ...
  def __init__(self, name, message, output_path):
    ...
    super(SaveMessageOp, self).__init__(
        ...
        command=['sh', '-c'],
        arguments=['echo \'%s\' | tee /tmp/results.txt | gsutil cp /tmp/results.txt \'%s\'' % (message, output_path)])
