...
type RunResourceReferenceSorter []*run_model.V1ResourceReference
...
func (s *RunApiTestSuite) TestRunApis() {
	...
	createRunRequest := &runparams.CreateRunParams{Body: &run_model.V1Run{
		...
		ResourceReferences: []*run_model.V1ResourceReference{
			{Key: &run_model.V1ResourceKey{Type: run_model.V1ResourceTypeEXPERIMENT, ID: helloWorldExperiment.ID},
				Name: helloWorldExperiment.Name, Relationship: run_model.V1RelationshipOWNER},
			{Key: &run_model.V1ResourceKey{Type: run_model.V1ResourceTypePIPELINEVERSION, ID: helloWorldPipelineVersion.ID},
	...
	createExperimentRequest := &experimentparams.CreateExperimentParams{Body: &experiment_model.V1Experiment{Name: "argument parameter experiment"}}
	...
	createRunRequest = &runparams.CreateRunParams{Body: &run_model.V1Run{
		...
		PipelineSpec: &run_model.V1PipelineSpec{
			WorkflowManifest: string(argParamsBytes),
			Parameters: []*run_model.V1Parameter{
				{Name: "param1", Value: "goodbye"},
				{Name: "param2", Value: "world"},
			},
		},
		ResourceReferences: []*run_model.V1ResourceReference{
			{Key: &run_model.V1ResourceKey{Type: run_model.V1ResourceTypeEXPERIMENT, ID: argParamsExperiment.ID},
				Relationship: run_model.V1RelationshipOWNER},
		},
	}}
	...
	createLongRunningRunRequest := &runparams.CreateRunParams{Body: &run_model.V1Run{
		Name:        "long running",
		Description: "this pipeline will run long enough for us to manually terminate it before it finishes",
		ResourceReferences: []*run_model.V1ResourceReference{
			{Key: &run_model.V1ResourceKey{Type: run_model.V1ResourceTypeEXPERIMENT, ID: helloWorldExperiment.ID},
				Relationship: run_model.V1RelationshipOWNER},
			{Key: &run_model.V1ResourceKey{Type: run_model.V1ResourceTypePIPELINEVERSION, ID: longRunningPipelineVersion.ID},
				Relationship: run_model.V1RelationshipCREATOR},
		},
	}}
...
func (s *RunApiTestSuite) checkTerminatedRunDetail(t *testing.T, runDetail *run_model.V1RunDetail, experimentId string, experimentName string, pipelineVersionId string, pipelineVersionName string) {
	...
	expectedRun := &run_model.V1Run{
		...
		ResourceReferences: []*run_model.V1ResourceReference{
			...
			{Key: &run_model.V1ResourceKey{Type: run_model.V1ResourceTypePIPELINEVERSION, ID: pipelineVersionId},
				Name: pipelineVersionName, Relationship: run_model.V1RelationshipCREATOR},
		},
...
func (s *RunApiTestSuite) checkHelloWorldRunDetail(t *testing.T, runDetail *run_model.V1RunDetail, experimentId string, experimentName string, pipelineVersionId string, pipelineVersionName string) {
	// Check workflow manifest is not empty
...
func (s *RunApiTestSuite) checkArgParamsRunDetail(t *testing.T, runDetail *run_model.V1RunDetail, experimentId string, experimentName string) {
	argParamsBytes, err := ioutil.ReadFile("../resources/arguments-parameters.yaml")
	assert.Nil(t, err)
	argParamsBytes, err = yaml.ToJSON(argParamsBytes)
	assert.Nil(t, err)
