...
2. Enable custom task controller and other feature flags for kfp-tekton
   ...
   kubectl patch cm feature-flags -n tekton-pipelines \
         -p '{"data":{"embedded-status": "full"}}'
   kubectl patch cm config-defaults -n tekton-pipelines \
         -p '{"data":{"default-timeout-minutes": "0"}}'
   ```

3. Install Kubeflow Pipelines with Tekton backend (`kfp-tekton`) `v1.6.1` [custom resource definitions](https://kubernetes.io/docs/concepts/extend-kubernetes/api-extension/custom-resources/)(CRDs).
   > Note: You can ignore the error `no matches for kind "Application" in version "app.k8s.io/v1beta1"` since it's a warning saying `application` CRD is not yet ready.
    ```shell
    kubectl apply --selector kubeflow/crd-install=true -f install/v1.6.1/kfp-tekton.yaml
    ```

4. Install Kubeflow Pipelines with Tekton backend (`kfp-tekton`) `v1.6.1` deployment
    ```shell
    kubectl apply -f install/v1.6.1/kfp-tekton.yaml
    ```
