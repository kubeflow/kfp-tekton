...
export interface VisualizationCreatorConfig extends ViewerConfig {
  ...
  onGenerate?: (visualizationArguments: string, source: string, type: V1VisualizationType) => void;
...
interface VisualizationCreatorState {
  ...
  code: string;
  source: string;
  selectedType?: V1VisualizationType;
}
...
class VisualizationCreator extends Viewer<VisualizationCreatorProps, VisualizationCreatorState> {
  ...
  public render(): JSX.Element | null {
    ...
    const isCustomTypeAndHasCode = selectedType === V1VisualizationType.CUSTOM && code.length > 0;
    const canGenerate =
        ...
        <FormControl style={{ width: '100%' }}>
          ...
          <Select
            ...
            onChange={(e: React.ChangeEvent<{ name?: string; value: unknown }>) => {
              this.setState({ selectedType: e.target.value as V1VisualizationType });
            }}
            disabled={isBusy}
          >
            {this.getAvailableTypes(allowCustomVisualizations).map((key: string) => (
              <MenuItem key={key} value={V1VisualizationType[key]}>
                {V1VisualizationType[key]}
              </MenuItem>
            ))}
          </Select>
        </FormControl>
        ...
        <BusyButton
          ...
          onClick={() => {
            if (onGenerate && selectedType) {
              ...
              if (selectedType === V1VisualizationType.CUSTOM) {
  ...
  /*
    ...
    For example:
    export enum V1VisualizationType {
      ROCCURVE = <any> 'ROC_CURVE'
    }

    Object.keys(V1VisualizationType) = ['ROCCURVE', 'ROC_CURVE'];
  ...
  */
  private getAvailableTypes(allowCustomVisualizations: boolean): string[] {
    return Object.keys(V1VisualizationType)
  ...
  private getArgumentPlaceholderForType(type: V1VisualizationType | undefined): string {
    let placeholder = 'Arguments, provided as JSON, to be used during visualization generation.';
    switch (type) {
      case V1VisualizationType.ROCCURVE:
        ...
        break;
      case V1VisualizationType.TFDV:
        placeholder = '{}';
        break;
      case V1VisualizationType.TFMA:
        placeholder = `{
        \t"slicing_column: string | null
        }`;
        break;
      case V1VisualizationType.TABLE:
        placeholder = '{\n\t"headers": array\n}';
        break;
      case V1VisualizationType.CUSTOM:
        placeholder = '{\n\t"key": any\n}';
        break;
    }
    return (
